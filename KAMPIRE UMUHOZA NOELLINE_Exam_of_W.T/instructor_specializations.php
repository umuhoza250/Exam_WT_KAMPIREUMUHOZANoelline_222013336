...
<section>
  <center>
 <center><h1><u>Instructor Specializations Form</u></h1></center>

    <form method="post" onsubmit="return confirmInsert();">
        <label for="specialization_id">Specialization ID:</label>
        <input type="number" id="specialization_id" name="specialization_id"><br><br>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO instructor_specializations(specialization_id, instructor_id, specialization) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $specialization_id, $instructor_id, $specialization);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail Information of Instructor Specializations</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of Instructor Specializations</h2></center>
    <table border="5">
        <tr>
        ...
        // Prepare SQL query to retrieve all instructor specializations
        $sql = "SELECT * FROM instructor_specializations";
        $result = $connection->query($sql);

        // Check if there are any instructor specializations
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $specialization_id = $row['specialization_id']; // Fetch the specialization ID
                echo "<tr>
                    <td>" . $row['specialization_id'] . "</td>
                    <td>" . $row['instructor_id'] . "</td>
                    <td>" . $row['specialization'] . "</td>
                    <td><a style='padding:4px' href='delete_specializations.php?specialization_id=$specialization_id'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_specializations.php?specialization_id=$specialization_id'>Update</a></td> 
                </tr>";
            }
        } else {
