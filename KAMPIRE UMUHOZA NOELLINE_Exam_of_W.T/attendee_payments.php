...
<head>
  ...
  <title>Attendee Payments entity Page</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
<section>
  <center>
   <center><h1><u>Attendee Payments Form</u></h1></center>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO attendee_payments(payment_id, attendee_id, amount_paid) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $payment_id, $attendee_id, $amount_paid);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail Information of Attendee Payments</title>
    <style>
        table {
            width: 100%;
...
<body>
    ...
    <center><h2>Table of Attendee Payments</h2></center>
    <table border="5">
        <tr>
            <th>Payment ID</th>
            <th>Attendee ID</th>
        ...
        // Prepare SQL query to retrieve all attendee payments
        $sql = "SELECT * FROM attendee_payments";
        $result = $connection->query($sql);

        // Check if there are any attendee payments
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr>
                    ...
                    <td><a style='padding:4px' href='delete_attendeepayments.php?payment_id=$payment_id'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_attendeepayments.php?payment_id=$payment_id'>Update</a></td> 
                </tr>";
            }
        } else {
