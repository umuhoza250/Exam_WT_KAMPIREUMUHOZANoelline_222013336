...
<head>
  ...
  <title>Attendees entity Page</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt->bind_param("iii", $attendee_id, $user_id, $workshop_id); // Changed "sss" to "iii"
...
<table border="5">
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $attendee_id = $row['attendee_id']; // Fetch the Attendee ID
            echo "<tr>
